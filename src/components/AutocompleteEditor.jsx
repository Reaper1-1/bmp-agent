...
export default function AutocompleteEditor() {
  ...
  function handleBlur() {
    // Allow suggestion clicks to process before closing the list
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto">
      ...
      <textarea
        ...
        aria-controls="as2-suggestions"
        aria-expanded={showSuggestions}
        aria-activedescendant={
          showSuggestions ? `as2-suggestion-${activeIndex}` : undefined
        }
        spellCheck={false}
      ...
      <div className="sr-only" aria-live="polite">
        {loading ? "Loading keywords" : `${keywords.length} keywords ready`}
      </div>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div
          ...
          className="absolute left-0 right-0 mt-1 max-h-56 overflow-auto border bg-white rounded-xl shadow-lg z-20"
        >
          {suggestions.map((kw, index) => (
            <button
              key={kw + index}
              id={`as2-suggestion-${index}`}
              type="button"
              onMouseDown={(e) => {
